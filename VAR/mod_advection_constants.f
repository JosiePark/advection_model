      module mod_advection_constants

      use mod_advection_input
      
      implicit none
      
      integer iseed,n,nrec,i,j,k,m,t,b
      
      real*8 uscale,tscale,scale
      
c Lagrangian particle positions      
      
      real*8 x0(npoints),y0(npoints),x1(npoints),y1(npoints),x2(npoints)
...
c npoints will now be the number of particles per release
     ...
     & ,x1r_disp,x2r_disp,y1r_disp,y2r_disp
...
c Names of time variables
      ...
      integer max_time_lag,t_start,release_start
...
c Bin variables
      ...
      real*8 PV_bin(nbins),d_bin,Y_bin(nbins),j_bin(jj)
      real*8 PV_bar(jj),rel1_av(ii,jj),rel2_av(ii,jj),zeta1_av(ii,jj)
     & ,zeta2_av(ii,jj) 
      
      end module mod_advection_constants
